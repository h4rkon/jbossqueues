...
public class KafkaMessageConsumer extends AbstractMessageConsumer<MessagePayload>
    ...
    void start() {
        if (!configuration.enabled()) {
            logger().info("Kafka consumer disabled. Configure KAFKA_BOOTSTRAP_SERVERS and KAFKA_TOPIC_MESSAGES to enable it.");
            return;
        }
    ...
    private void consumeRecord(ConsumerRecord<String, String> record) {
        try {
            ...
            logger().log(Level.SEVERE, "Kafka message processing failed; sending to DLQ", exception);
